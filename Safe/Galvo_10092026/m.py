...
class MainWindow(QMainWindow):
    ...
    def setupConfigGalvo(self):
        widget_names = [
            "config_field_size", "config_offset_x", "config_offset_y", "config_angle",
            "config_use_corfile", "config_corfile_path", "config_corfile_btn",
            "config_galvo1_x", "config_galvo2_x",
            "config_g1_negate", "config_g1_scale", "btn_g1_scale", "config_g1_bulge", "config_g1_skew", "config_g1_trapezoid",
            "config_g2_negate", "config_g2_scale", "btn_g2_scale", "config_g2_bulge", "config_g2_skew", "config_g2_trapezoid",
            "config_pos_NoMovement", "config_pos_GalvoCenter", "config_pos_TopLeft", "config_pos_TopRight", 
            "config_pos_BottomRight", "config_pos_BottomLeft", "config_pos_Specialpos", "config_sp_x", "config_sp_y",
            "config_btn_ok", "config_btn_cancel", "config_btn_apply"
        ]
        
        widget_list = []
        for w in widget_names:
            if hasattr(self.ui, w):
                ui_widget = getattr(self.ui, w)
                w_type = type(ui_widget)
                if w_type in (QCheckBox, QRadioButton, QPushButton):
                    signal = "clicked"
                else:
                    signal = "textChanged"
                
                widget_obj = self.wh.configWidget(self, w_type, w, signal, self.configGalvoAction, role=w)
                if widget_obj:
    ...
    def setupHeader(self):
        ...
        self.wh.configWidget(self, QPushButton, "minimizePushButton", "clicked", self.minimizeWindow, role="minimize", toolTip="Minimize")
        self.wh.configWidget(self, QPushButton, "restorePushButton", "clicked", self.restoreWindow, role="restore", toolTip="Maximize")
        self.wh.configWidget(self, QPushButton, "closePushButton", "clicked", self.closeWindow, role="close", toolTip="Close")
    ...
    def initConfigGalvo(self):
        ...
        def _get(key, default): return self.ch.getValues("GalvoField", key) or default
        
        self.wh.invokeMethod(self.configgalvo_widgets.config_field_size, "set", _get("field_size", "100.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_offset_x, "set", _get("offset_x", "0.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_offset_y, "set", _get("offset_y", "0.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_angle, "set", _get("angle", "0.0"))
        
        is_g1_x = _get("galvo1_x", "True") == "True"
        self.ui.config_galvo1_x.setChecked(is_g1_x)
        self.ui.config_galvo2_x.setChecked(not is_g1_x)
        
        self.ui.config_use_corfile.setChecked(_get("use_corfile", "False") == "True")
        self.wh.invokeMethod(self.configgalvo_widgets.config_corfile_path, "set", _get("corfile_path", ""))
        
        self.ui.config_g1_negate.setChecked(_get("g1_negate", "False") == "True")
        self.wh.invokeMethod(self.configgalvo_widgets.config_g1_scale, "set", _get("g1_scale", "100.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_g1_bulge, "set", _get("g1_bulge", "1.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_g1_skew, "set", _get("g1_skew", "1.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_g1_trapezoid, "set", _get("g1_trapezoid", "1.0"))
        
        self.ui.config_g2_negate.setChecked(_get("g2_negate", "False") == "True")
        self.wh.invokeMethod(self.configgalvo_widgets.config_g2_scale, "set", _get("g2_scale", "100.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_g2_bulge, "set", _get("g2_bulge", "1.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_g2_skew, "set", _get("g2_skew", "1.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_g2_trapezoid, "set", _get("g2_trapezoid", "1.0"))
        
        pos_mode = _get("pos_after_mark", "config_pos_NoMovement")
        if hasattr(self.ui, pos_mode): getattr(self.ui, pos_mode).setChecked(True)
        self.wh.invokeMethod(self.configgalvo_widgets.config_sp_x, "set", _get("sp_x", "0.0"))
        self.wh.invokeMethod(self.configgalvo_widgets.config_sp_y, "set", _get("sp_y", "0.0"))
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Yes:
            ...
            self.shutdownLaser()
            self.logger.info("Lasers Shutdown")
    ...
    def jogGalvoAction(self, widget, *args):
        ...
        if action in ["galvohome", "galvozhome", "zplusgalvo", "zminusgalvo"]:
            if not self.galvo_controller.is_connected:
    ...
    def load_galvo_test_pattern(self, shape):
        self._current_test_shape = shape
        self.planner.clear()
        try:
        ...
        size_gu = int(min(100 * galvo_units_per_mm, 65535))
        half_size_gu = size_gu // 2
        cx, cy = 32767, 32767

        if shape == "square":
            self.planner.add_jump(cx - half_size_gu, cy - half_size_gu, speed=jump_speed)
            self.planner.add_mark(cx + half_size_gu, cy - half_size_gu, speed=mark_speed)
            self.planner.add_mark(cx + half_size_gu, cy + half_size_gu, speed=mark_speed)
            self.planner.add_mark(cx - half_size_gu, cy + half_size_gu, speed=mark_speed)
            self.planner.add_mark(cx - half_size_gu, cy - half_size_gu, speed=mark_speed)
        elif shape == "circle":
            radius = int(50 * galvo_units_per_mm)
            for i in range(37):
                angle = math.radians(i * 10)
                px = cx + int(radius * math.cos(angle))
                py = cy + int(radius * math.sin(angle))
                if i == 0:
                    self.planner.add_jump(px, py, speed=jump_speed)
                else:
                    self.planner.add_mark(px, py, speed=mark_speed)
        elif shape == "triangle":
            self.planner.add_jump(cx, cy + half_size_gu, speed=jump_speed)
            self.planner.add_mark(cx + half_size_gu, cy - half_size_gu, speed=mark_speed)
            self.planner.add_mark(cx - half_size_gu, cy - half_size_gu, speed=mark_speed)
            self.planner.add_mark(cx, cy + half_size_gu, speed=mark_speed)
            
        self.planner.add_jump(cx, cy, speed=jump_speed)
    ...
    def laserConfGalvoAction(self, widget, *args):
        ...
        if action == "ftravelgalvo":
            ...
            self.showAutoCloseMessage("Focus Set", f"Z Focus position saved at {focus_pos:.3f} mm.")
    ...
    def programsGalvoAction(self, widget, *args):
        ...
        if action == "pgmfilegalvo":
            self.pgm_file, _ = QFileDialog.getOpenFileName(self, "Select File", "", "SVG Files (*.svg);;DXF Files (*.dxf)")
            if self.pgm_file:
    ...
    def _apply_galvo_settings(self):
        if hasattr(self, 'pgm_file') and self.pgm_file:
    ...
    def process_galvo_file(self):
        ...
        target_min_x = start_x - (lens_size / 2)
        target_min_y = start_y - (lens_size / 2)
    ...
    def printGalvoAction(self, widget, *args):
        ...
        if action == "redlightpre":
            ...
            try:
                ...
                # The user wants reverse voltage: 0 -> 9.8V, 100 -> 0V.
                slider_value = 100.0 - slider_percentage
    ...
    def configGalvoAction(self, widget, *args):
        ...
        if action == "config_corfile_btn":
            file_path, _ = QFileDialog.getOpenFileName(self, "Select Correction File", "", "Cor Files (*.cor);;All Files (*.*)")
            if file_path:
                self.wh.invokeMethod(self.configgalvo_widgets.config_corfile_path, "set", file_path)
                
        elif action in ["config_btn_apply", "config_btn_ok"]:
            # Save all to config
            def _save(key, val): self.ch.addParameter("GalvoField", key, str(val))
            
            _save("field_size", self.wh.invokeMethod(self.configgalvo_widgets.config_field_size, "get"))
            _save("offset_x", self.wh.invokeMethod(self.configgalvo_widgets.config_offset_x, "get"))
            _save("offset_y", self.wh.invokeMethod(self.configgalvo_widgets.config_offset_y, "get"))
            _save("angle", self.wh.invokeMethod(self.configgalvo_widgets.config_angle, "get"))
            
            _save("galvo1_x", self.ui.config_galvo1_x.isChecked())
            _save("use_corfile", self.ui.config_use_corfile.isChecked())
            _save("corfile_path", self.wh.invokeMethod(self.configgalvo_widgets.config_corfile_path, "get"))
            
            _save("g1_negate", self.ui.config_g1_negate.isChecked())
            _save("g1_scale", self.wh.invokeMethod(self.configgalvo_widgets.config_g1_scale, "get"))
            _save("g1_bulge", self.wh.invokeMethod(self.configgalvo_widgets.config_g1_bulge, "get"))
            _save("g1_skew", self.wh.invokeMethod(self.configgalvo_widgets.config_g1_skew, "get"))
            _save("g1_trapezoid", self.wh.invokeMethod(self.configgalvo_widgets.config_g1_trapezoid, "get"))
            
            _save("g2_negate", self.ui.config_g2_negate.isChecked())
            _save("g2_scale", self.wh.invokeMethod(self.configgalvo_widgets.config_g2_scale, "get"))
            _save("g2_bulge", self.wh.invokeMethod(self.configgalvo_widgets.config_g2_bulge, "get"))
            _save("g2_skew", self.wh.invokeMethod(self.configgalvo_widgets.config_g2_skew, "get"))
            _save("g2_trapezoid", self.wh.invokeMethod(self.configgalvo_widgets.config_g2_trapezoid, "get"))
            
            # Find checked radio
            radios = ["config_pos_NoMovement", "config_pos_GalvoCenter", "config_pos_TopLeft", "config_pos_TopRight", "config_pos_BottomRight", "config_pos_BottomLeft", "config_pos_Specialpos"]
            for r in radios:
                if getattr(self.ui, r).isChecked():
                    _save("pos_after_mark", r)
                    break
                    
            _save("sp_x", self.wh.invokeMethod(self.configgalvo_widgets.config_sp_x, "get"))
            _save("sp_y", self.wh.invokeMethod(self.configgalvo_widgets.config_sp_y, "get"))
            ...
            if hasattr(self, 'pgm_file') and self.pgm_file:
                self.process_galvo_file()
            elif hasattr(self, '_current_test_shape') and self._current_test_shape:
                self.load_galvo_test_pattern(self._current_test_shape)
                
            self.showAutoCloseMessage("Config Saved", "Galvo configuration has been saved and applied.")
            
            if action == "config_btn_ok":
                # Close/hide config logic? User was just on this page, maybe redirect?
                self.showMainPages("printgalvo")

    def _apply_galvo_config_to_core(self):
        try:
            self.galvo_config.field_size = float(self.wh.invokeMethod(self.configgalvo_widgets.config_field_size, "get") or 100.0)
            self.galvo_config.offset_x = float(self.wh.invokeMethod(self.configgalvo_widgets.config_offset_x, "get") or 0.0)
            self.galvo_config.offset_y = float(self.wh.invokeMethod(self.configgalvo_widgets.config_offset_y, "get") or 0.0)
            self.galvo_config.angle = float(self.wh.invokeMethod(self.configgalvo_widgets.config_angle, "get") or 0.0)
            self.galvo_config.galvo_1_is_x = self.ui.config_galvo1_x.isChecked()
            
            self.galvo_config.g1_negate = self.ui.config_g1_negate.isChecked()
            self.galvo_config.g1_scale = float(self.wh.invokeMethod(self.configgalvo_widgets.config_g1_scale, "get") or 100.0)
            self.galvo_config.g1_bulge = float(self.wh.invokeMethod(self.configgalvo_widgets.config_g1_bulge, "get") or 1.0)
            self.galvo_config.g1_skew = float(self.wh.invokeMethod(self.configgalvo_widgets.config_g1_skew, "get") or 1.0)
            self.galvo_config.g1_trapezoid = float(self.wh.invokeMethod(self.configgalvo_widgets.config_g1_trapezoid, "get") or 1.0)
            
            self.galvo_config.g2_negate = self.ui.config_g2_negate.isChecked()
            self.galvo_config.g2_scale = float(self.wh.invokeMethod(self.configgalvo_widgets.config_g2_scale, "get") or 100.0)
            self.galvo_config.g2_bulge = float(self.wh.invokeMethod(self.configgalvo_widgets.config_g2_bulge, "get") or 1.0)
            self.galvo_config.g2_skew = float(self.wh.invokeMethod(self.configgalvo_widgets.config_g2_skew, "get") or 1.0)
            self.galvo_config.g2_trapezoid = float(self.wh.invokeMethod(self.configgalvo_widgets.config_g2_trapezoid, "get") or 1.0)
        except Exception as e:
            self.util.debugPrint(f"Error applying config to core: {e}")
            ...
            if hasattr(self, 'master_glist') and self.master_glist:
                ...
                for line in base_glist:
                    if "X" in line or "Y" in line:
                        def rx(m): return f"X{float(m.group(1)) + cam_ox:.3f}"
                        def ry(m): return f"Y{float(m.group(1)) + cam_oy:.3f}"
    ...
    def resetProfile(self):
        self.wh.invokeMethod(self.program_widgets.pgmheight, "clear")
        self.wh.invokeMethod(self.program_widgets.pgmfeed, "clear")
        self.wh.invokeMethod(self.program_widgets.pgmlaser, "clear")
        self.wh.invokeMethod(self.program_widgets.pgmfile, "clear")
        
        self.wh.invokeMethod(self.program_widgets.pgmheightset, "disable")
        self.wh.invokeMethod(self.program_widgets.pgmfeedset, "disable")
        self.wh.invokeMethod(self.program_widgets.pgmlaserset, "disable")
        self.wh.invokeMethod(self.program_widgets.pgmsaveset, "disable")
